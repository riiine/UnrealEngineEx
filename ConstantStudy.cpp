...
void AConstantStudy::BeginPlay()
{
	...
	*/
	...
	그리고 IntPtr1포인터 변수가 가리키는 값 (*IntPtr1)을 상수화했습니다. 
	*/
	...
	*IntPtr1 = 2;는 IntPtr1이 cons 변수이기 때문에 컴파일 에러가 발생하지만
	...
	*/
	...
	IntValue6변수의 주소값을 IntPtr3라는 이름의 포인터 변수에 대입함으로써 초기화 합니다. 
	...
	*/
	int32 Num = 10;
	/** 2 문장을 같은 문장입니다. 참조자를 사용하면 가독성이 좋습니다. */
	...
	#define(매크로),  const(상수), enum(열거형) 세가지 상수 선언 방식에 대해서 알아 봅니다. 
	*/

	/** 
	#define(매크로) 상수에 대해서 알아 봅니다. 
	소스코드를 빌드한다는 것은
	전처리(preprocess) -> 컴파일(compile) -> 어셈블(assemble) -> 링크(link)라는 일렬의 과정을 통해
	...
	*/
	...
	여기서 #define 은 매크로라고 하면 이를 이용해서 상수를 정의할 수 있습니다. 
...
#define HEIGHT 4
	...
	*/
	...
	2. 디테일 패널에 변수의 값이 노출되려면 컴포넌트를 써야 합니다. 값 변경이 필요하다면 디테일 패널의 
		변수값을 수정해 주면 됩니다. 몬스터들에는 이 방법을 적용합니다. ActorComponent
	*/
	...
	*/
	...
	const는 컴파일 시간, 란타임 시간에 모두 만들 수 있고
	...
	*/
	...
	/** 
	언리얼에서는 어쩔 수 없이 블루프린트를 써야 하는 부분이 2 부분이 있습니다. 
	1. 애니메이션 블루프린트의 AnimGraph 부분입니다. EventGraph부분은 코드로 합니다. 
	2. BehaviorTree의 기본 구조는 블루프린트로 합니다. 
		태스크, 서비스, 데코레이터는 코드로 합니다. 
	*/

	/** 지금까지 보았던 모든 포인터는 상수(Constant)가 아닌 값을 가리키는 비-상수입니다. */
	int32 IntValue12 = 5;
	int32* IntPtr12 = &IntValue12;
	*IntPtr12 = 6;

	/**
	하지만 값이 상수인 경우에는 어떻게 될까요?
	컴파일이 되지 않습니다. 상수 변수는 값을 변경할 수 없습니다.
	만약 상수가 아닌 포인터가 상수 변수를 가리킨 다음에 역참조하여 값을 변경할 수 있다면
	const의 의도를 위반하게 되므로 상수가 아닌 포인터는 상수를 가리킬 수가 없습니다.
	*/
	const int32 IntValue13 = 5;
	// int32* IntPtr13 = &IntValue13; // (X)
	// *IntPtr13 = 6;

	/**
	상수를 가리키는 포인터는 상수 변수의 주소를 가리키는 비-상수 포인터입니다.
	상수 변수에 대한 포인터를 선언하려면 자료형 앞에 const키워드를 사용하면 됩니다.
	*/
	const int32 IntValue14 = 5;
	const int32* IntPtr14 = &IntValue14;
	/** IntPtr14 포인터 변수가 가리키는 값을 상수화 했습니다. */
	//*IntPtr14 = 6; // (X)
	...
	/**
	비-상수 포인터는 상수 변수를 가리킬 수가 없습니다.

	상수 변수에 대한 포인터는 상수가 아닌 변수를 가리킬 수 있습니다.
	상수 변수에 대한 포인터는 변수가 초기에 const로 정의되었는지에 관계 없이 포인터를 통해
	접근할 때 변수를 상수로 취급합니다.
	*/
	int32 IntValue15 = 5;
	...
	/** 상수를 가리키는 포인터는 상수를 가리킬 뿐, 상수 자체가 아니므로 다른 값을 가리킬 수가 없습니다. */
	int32 IntValue17 = 5;
	const int32* IntPtr17 = &IntValue17;
	int32 IntValue18 = 6;
	/** 포인터 변수가 가리키는 값을 상수화했지, 포인터 변수 자체를 상수화하지 않았습니다. */
	IntPtr17 = &IntValue18;

	/**
	포인터 자체를 상수로 만들 수 있습니다.
	상수 포인터는 초기화 한 후에 가리키는 주소를 변경할 수 없는 포인터입니다.
	상수 포인터를 선언하려면 자료형 뒤와 변수 앞에 const키워드를 사용하면 됩니다.
	*/
	int32 IntValue19 = 5;
	int32* const IntPtr19 = &IntValue19;
	/** 포인터 변수 자체를 상수화 했기 때문에 다른 대상을 가리킬 수가 없습니다. */
	// IntPtr19 = &IntValue18; // (X)

	/** 포인트가 상수일 뿐, 가리키는 변수는 상수가 아니므로, 포인터를 역참조해서 값을 변경하는 것은 가능합니다. */
	int32 IntValue22 = 5;
	int32* const IntPtr22 = &IntValue22;
	*IntPtr22 = 6;

	/**
	자료형 앞 뒤에 const키워드를 사용해서 상수를 가리키는 상수 포인터를 선언할 수 있습니다.
	상수를 가리키는 상수 포인터는 다른 주소를 가리킬 수가 없습니다.
	역참조 연산자를 통해 값을 수정할 수도 없습니다.
	*/
	int32 IntValue23 = 5;
	int32 IntValue24 = 6;
	const int32* const IntPtr23 = &IntValue23;
	// *IntPtr23 = 10; // (X)
	// IntPtr23 = &IntValue24; // (X)

	/**
	함수 끝에 const키워드를 붙이면 어떻게 될까요?
...
/** 시력값을 반환하는 GetEyeSight라는 이름의 함수를 정의합니다. */
int32 Animal::GetEyeSight(const int EyeValue) const
{
	/** 매개변수에 있는 const지정어는 매개변수로 전달하는 값을 변경하지 못한다는 것을 보장합니다. */
	// EyeValue = 100; // (X)

	/**
	함수 끝에 붙는 const 지정어는 함수 안에서 멤버 변수를 변경하지 못한다는 것을 보장합니다.
	*/
	// EyeSight = 200; // (X)
	return EyeSight;
}
